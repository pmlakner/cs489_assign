...
g_optimizer = optim.Adam(g_params, opts.lr, [opts.beta1, opts.beta2])
    ...
    for iteration in range(1, opts.train_iters+1):
        ...
        D_X_loss = (D_X.forward(images_X) - 1).pow(2).mean()
        D_Y_loss = (D_Y.forward(images_Y) - 1).pow(2).mean()
        ...
        D_X_loss = D_X.forward(fake_X).pow(2).mean()
        
        # 4. Generate fake images that look like domain Y based on real images in domain X
        fake_Y = G_XtoY(images_X)
        
        # 5. Compute the loss for D_Y
        D_Y_loss = D_Y.forward(fake_Y).pow(2).mean()
        ...
        g_loss = (D_X.forward(fake_X) - 1).pow(2).mean()
        ...
        g_loss = (D_Y.forward(fake_Y) - 1).pow(2).mean()
